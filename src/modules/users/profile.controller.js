...
export const completeProfile = asyncHandler(async (req, res) => {
    ...
    if (!ALLOWED_ROLES.has(role)) {
        return failure(res, "users.profile.invalidRole");
    }

    if (yob && (yob < 1900 || yob > new Date().getFullYear())) {
        return failure(res, "users.profile.invalidYob");
    }
    ...
    return success(res, "users.profile.setupComplete", {
        role_primary: role,
        profile,
...
export const getMyProfile = asyncHandler(async (req, res) => {
    ...
    return success(res, "users.profile.retrieved", {
        ...u,
    ...
    try {
        ...
        if (!currentProfile[0]) {
         ...
         FROM app.expert_profiles WHERE user_id = $1`,
            ...
            return success(res, "experts.profile.created", newProfile[0]);
        }
        ...
        return failure(res, "experts.profile.updateFailed");
    }
});
